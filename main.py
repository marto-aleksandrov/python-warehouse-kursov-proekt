...
class Product:
    ...
    def to_dict(self):
        """Връща стоката като речник (за запазване във файл)."""
        return {
            "name": self.name,
    ...
    def from_dict(cls, data):
        """Създава Product от речник (при зареждане от файл)."""
...
class Warehouse:
    ...
    def remove_product(self, name):
        """Премахва стока по име. Връща True, ако е намерена и премахната."""
        for product in self.products:
    ...
    def save_to_file(self, path):
        """Запазва всички стоки във файл във формат JSON."""
        data = [product.to_dict() for product in self.products]
        with open(path, "w", encoding="utf-8") as f:
            json.dump(data, f, ensure_ascii=False, indent=2)

    def load_from_file(self, path):
        """Зарежда стоките от JSON файл, като замества текущите."""
